...
class Main():
    def main(self,model_type,model_name_or_path,length, temperature):
        self.model_type=model_type
        self.model_name_or_path=model_name_or_path
        self.prompt=str(input('Input the sentence: >>'))
        self.padding_text=""
        self.xlm_lang=""
        self.length=length
        self.num_samples=1
    ...
    #                        help="primarily useful for CTRL model; in that case, use 1.2")
     ##  parser.add_argument("--top_p", type=float, default=0.9)
       # parser.add_argument("--no_cuda", action='store_true',
        ...
        if self.length < 0 and model.config.max_position_embeddings > 0:
            self.length = model.config.max_position_embeddings
        elif 0 < model.config.max_position_embeddings < self.length:
        ...
        while True:
            ...
            if self.model_type in ["xlm"] and hasattr(tokenizer, 'lang2id') and hasattr(model.config, 'use_lang_emb') \
                    and model.config.use_lang_emb:
                if self.xlm_lang:
                    language = self.xlm_lang
                else:
                    language = None
            ...
            is_xlm_mlm = self.model_type in ["xlm"] and 'mlm' in self.model_name_or_path
            if is_xlm_mlm:
                xlm_mask_token = tokenizer.mask_token_id
            else:
                xlm_mask_token = None

            raw_text = self.prompt if self.prompt else input("Model prompt >>> ")
